...
app.get('/vlc-redirect', (req, res) => {
  ...
  console.log(`vlc-redirect received mrl: ${mrl}`);
  ...
  console.log(`vlc-redirect stripped mrl: ${mrl}`);

  // If the MRL is already a proxy URL generated by the frontend, redirect directly to it
  if (mrl.includes('/vlc-proxy/')) {
    console.log(`vlc-redirect redirecting to: ${mrl}`);
    return res.redirect(mrl);
  }
...
app.get('/vlc-proxy/:b64mrl/:filename', async (req, res) => {
  ...
  try {
    ...
    console.log(`vlc-proxy fetching mrl: ${mrl}`);
